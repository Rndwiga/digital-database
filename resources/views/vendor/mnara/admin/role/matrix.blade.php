...
@section('content')
    <div class="right_col" role="main">
        ...
        {!! Form::open( [ 'route' => [ config('mnara.route.as') .'role.matrix' ], 'class' => 'form-horizontal'] ) !!}
        <div class="table" style="max-height:350px; overflow:auto; border: 1px dashed;">
            <table class="table table-bordered table-striped table-hover" style=" margin-bottom:0">
                ...
                <tr class="alert-warning">
                    ...
                    @foreach ($roles as $r)
                        <th> {{ $r->name }} <a href="{{ route( config('mnara.route.as') .'role.show',$r->id) }}">
                ...
                @foreach($perms as $p)
                    <tr>
                        <th class="alert-warning">
                            <a href="{{ route('mnara.permission.show',$p->id) }}">
                                <button type="button" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-link"></span></button></a>
                            {{ $p->name }}
                        </th>
